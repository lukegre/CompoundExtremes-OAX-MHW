...
rcMaps = {
    ...
    "proj": crs.EqualEarth(central_longitude=205),
    "land_color": "#dddddd",
    "coast_res": "110m",
    "coast_lw": 0.5,
    "robust": True,
    ...
    "colorbar.aspect": 15,
    "colorbar.shrink": 0.45,
    "colorbar.extendfrac": 0.05,
}
...
def ndimage_fill_holes(da, shape=[1, 3, 3]):
...
def plot_contours(regions, ax, **kwargs):
    ...
    props = (
        dict(
            ...
            colors=["k"],
            linewidths=1,
            levels=np.arange(0.5, 6),
        )
        | kwargs
    )

    img = fill_lon_gap(regions.load()).plot.contour(**props)
